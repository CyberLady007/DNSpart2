...
def run_dns_server():
    ...
    while True:
        try:
            ...
            # Handle specific DNS queries
            if qname == 'nyu.edu.' and qtype == dns.rdatatype.TXT:
                # Retrieve the encrypted data from the dns_records dictionary
                encrypted_data = dns_records[qname][qtype]
                decrypted_data = decrypt_with_aes(encrypted_data, password, salt)
                # Create an rdata object with the decrypted data
                rdata = dns.rdata.from_text(dns.rdataclass.IN, qtype, decrypted_data)
                ...
                # Handle other types of DNS queries (A, MX, etc.) as you were doing before
                # ...
