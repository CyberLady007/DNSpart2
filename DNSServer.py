...
def encrypt_with_aes(input_string, password, salt):
    key = generate_aes_key(password, salt)  # Use the correct parameters
    f = Fernet(key)
...
dns_records = {
    ...
    'nyu.edu.': {
        ...
        dns.rdatatype.TXT: encrypt_with_aes("hello",password,salt),
...
def run_dns_server():
    ...
    server_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)  # Use correct socket type
    server_socket.bind(('127.0.0.1', 53))  # Use correct IP address and port
    ...
    while True:
        try:
            ...
            server_socket.sendto(response.to_wire(), addr)  # Correctly send the response
